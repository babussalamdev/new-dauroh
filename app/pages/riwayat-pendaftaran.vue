<template>
  <div class="container py-5">
      ...
      <div v-else-if="sortedTickets.length > 0" class="table-responsive">
        <table class="table table-hover align-middle mb-0">
          <thead class="bg-light">
            <tr>
              <th scope="col" class="ps-4 py-3 text-uppercase text-secondary fs-7 fw-semibold text-center">Tanggal & ID
              </th>
              ...
              <th scope="col" class="pe-4 py-3 text-uppercase text-secondary fs-7 fw-semibold text-center"
            ...
            <tr v-for="(ticket, index) in sortedTickets" :key="ticket.SK || index">
              <td class="ps-4 py-3">
                <div class="d-flex flex-column">
                  <span class="fw-bold text-dark mb-1">{{ formatDate(ticket.CreatedAt || ticket.created_at ||
                    ticket.date) }}</span>
              ...
              <td class="py-3" style="max-width: 200px;">
                <span class="d-block fw-bold text-primary text-truncate mb-1"
                  :title="ticket.Title || ticket.title || ticket.event?.Title">
              ...
              <td class="py-3 text-center">
                <div class="d-flex flex-column align-items-center gap-1">
                  <template v-if="getSmartStatus(ticket) === 'SUCCESSFUL'">
                    <span
                      class="badge rounded-pill bg-success-subtle text-success border border-success-subtle px-2 py-1 small-7">
                  ...
                  <template v-else-if="['CANCELLED', 'EXPIRED', 'FAILED'].includes(getSmartStatus(ticket))">
                    <span
                      class="badge rounded-pill bg-danger-subtle text-danger border border-danger-subtle px-2 py-1 small-7">
                  ...
                  <template v-else-if="getSmartStatus(ticket) === 'PENDING'">
                    <span
                      class="badge rounded-pill bg-warning-subtle text-warning border border-warning-subtle px-2 py-1 small-7 mb-1">
              ...
              <td class="pe-4 py-3 text-center">
                <div class="d-flex flex-column gap-2 mx-auto" style="max-width: 120px;">
                  ...
                  <button @click="openDetailModal(ticket)"
    ...
    <div v-if="showDetail" class="modal fade show d-block" style="background-color: rgba(0,0,0,0.5);" tabindex="-1"
        ...
        <div class="modal-content rounded-4 border-0 shadow-lg">
          ...
          <div class="modal-body">
            <div v-if="selectedTicketDetail" class="mb-3">
              <h6 class="text-primary fw-bold mb-1">{{ selectedTicketDetail.Title || selectedTicketDetail.title ||
                selectedTicketDetail.event?.Title }}</h6>
                ...
                <tbody>
                  <tr v-for="(p, idx) in selectedTicketDetail.participants" :key="idx">
                    ...
                    <td class="text-end pe-3">
                      <button v-if="getSmartStatus(selectedTicketDetail) === 'SUCCESSFUL'"
                        @click="showIndividualQr(selectedTicketDetail, p)"
                        class="btn btn-sm btn-primary py-1 px-3 rounded-pill" style="font-size: 0.75rem;">
            ...
            <div v-if="selectedTicketDetail && getSmartStatus(selectedTicketDetail) !== 'SUCCESSFUL'"
              class="alert alert-warning d-flex align-items-center mt-3 mb-0 p-2 small">
