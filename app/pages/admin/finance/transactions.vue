        ...
        <div class="row g-3 align-items-end">
          ...
          <div class="col-12 col-md-4 col-lg-4">
            ...
            <div class="input-group input-group-sm shadow-sm">
              ...
              <input type="text" class="form-control form-control-sm bg-light border-0 ps-0" placeholder="No. Invoice / Nama..." v-model="searchQuery">
            </div>
          </div>
          
          <div class="col-12 col-md-3 col-lg-3">
            <label class="fw-bold text-muted mb-1" style="font-size: 0.75rem; letter-spacing: 0.5px;">PILIH EVENT</label>
            <select class="form-select form-select-sm bg-light border-0 shadow-sm" v-model="filterEvent">
          ...
          <div class="col-8 col-md-3 col-lg-3">
            ...
            <select class="form-select form-select-sm bg-light border-0 shadow-sm" v-model="filterStatus">
          ...
          <div class="col-4 col-md-2 col-lg-2">
            <button class="btn btn-light border btn-sm w-100 fw-bold shadow-sm" @click="resetFilters">
    ...
    <div class="card content-card border-0 shadow-sm rounded-4 mb-4">
      <div class="card-header bg-white p-3 px-md-4 py-md-3 border-bottom d-flex justify-content-between align-items-center">
        <h5 class="mb-0 fw-bold text-dark fs-6 fs-md-5">
          <i class="bi bi-receipt text-primary me-2"></i>Riwayat Transaksi
        </h5>
        <span class="badge bg-primary bg-opacity-10 text-primary px-3 py-2 rounded-pill shadow-sm" style="font-size: 0.75rem;">
      ...
      <div class="card-body p-0">
        ...
        <div class="table-responsive" v-else>
          <table class="table table-hover mb-0" style="min-width: 900px;">
            <thead>
              <tr>
                <th class="ps-4 text-nowrap">INVOICE & TANGGAL</th>
                <th class="text-nowrap">INFORMASI PEMBELI</th>
                <th class="text-nowrap">NOMINAL TRANSAKSI</th>
            ...
            <tbody>
              <tr v-for="tx in filteredTransactions" :key="tx.invoice">
                <td class="ps-4">
                  ...
                  <div class="text-muted mt-1 small"><i class="bi bi-calendar2-event me-1"></i>{{ dayjs(tx.date).format('DD MMM YYYY, HH:mm') }} WIB</div>
                </td>
                <td>
                  <div class="fw-bold text-dark">{{ tx.buyerName }}</div>
                  <div class="text-muted text-truncate mt-1 small" style="max-width: 250px;">{{ tx.eventName }}</div>
                </td>
                <td>
                  <div class="fw-bold text-success">{{ formatRupiah(tx.totalAmount) }}</div>
                  <div class="text-muted mt-1 small">{{ tx.qty }} Tiket Event</div>
                </td>
                <td>
                  <span class="badge bg-light text-dark border fw-medium px-2 py-1"><i class="bi bi-bank me-1 text-muted"></i>{{ tx.paymentMethod }}</span>
                </td>
                <td class="text-center">
                  <span v-if="tx.status === 'LUNAS'" class="badge bg-success bg-opacity-10 text-success border border-success px-3 py-1 rounded-pill shadow-sm" style="font-size: 0.75rem;"><i class="bi bi-check-circle-fill me-1"></i> LUNAS</span>
                  <span v-else-if="tx.status === 'PENDING'" class="badge bg-warning bg-opacity-10 text-dark border border-warning px-3 py-1 rounded-pill shadow-sm" style="font-size: 0.75rem;"><i class="bi bi-clock-fill text-warning me-1"></i> PENDING</span>
                  <span v-else class="badge bg-danger bg-opacity-10 text-danger border border-danger px-3 py-1 rounded-pill shadow-sm" style="font-size: 0.75rem;"><i class="bi bi-x-circle-fill me-1"></i> EXPIRED</span>
                </td>
                <td class="text-center pe-4">
                  <button class="btn btn-sm text-primary p-0 border-0 bg-transparent shadow-none" @click="openDetail(tx)" title="Lihat Detail">
              ...
              <tr v-if="filteredTransactions.length === 0">
                <td colspan="6" class="text-center py-5 text-muted">
                  <i class="bi bi-receipt fs-1 opacity-50 mb-3 d-block"></i>
